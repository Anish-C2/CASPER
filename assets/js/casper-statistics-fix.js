  ...
  function statisticsPageFixed() {
    ...
    ms.forEach(function (x) {
      ...
      if (m.kind === 'cricket') runs += Number(m.sh || 0) + Number(m.sa || 0);
        ...
        goals += Number(m.sh || 0) + Number(m.sa || 0);
        if (m.sh === 0 || m.sa === 0) clean++;
      }
      if (m.p) pens++;
    });
    ps.forEach(function (p) { hats += p.hats; });

    var scorer = ps.filter(function (p) { return p.goals > 0; })
      .sort(function (a, b) { return b.goals - a.goals || b.assists - a.assists; }).slice(0, 10);
    var runner = ps.filter(function (p) { return p.matches >= 3; })
      .sort(function (a, b) { return (b.wins / b.matches) - (a.wins / a.matches) || b.wins - a.wins; }).slice(0, 10);
    ...
    var namedMap = {};
    ms.forEach(function (x) {
      ['gh', 'ga'].forEach(function (field) {
        (x.m[field] || []).forEach(function (g) {
          var name = String(g.name || '').trim(), k = name.toLowerCase();
          if (!k) return;
          if (!namedMap[k]) namedMap[k] = { name: name, goals: 0 };
          namedMap[k].goals += Number(g.n || 0);
        });
      });
    });
